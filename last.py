...
COLUMN_MAPPING = {
    # 标准列名: 可能的变体列表
    '学号': ['学号', '学生编号', 'ID', 'student_id', '学生ID'],
    '性别': ['性别', '男/女', 'gender'],
    '专业': ['专业', '所学专业', 'major', '学院'],
    '每周学习时长': ['每周学习时长', '学习时长', '每周学习时间', 'study_hours', '学习时长(小时)'],
    '上课出勤率': ['上课出勤率', '出勤率', '出勤比例', 'attendance', '上课出勤'],
    '期中考试分数': ['期中考试分数', '期中分数', 'midterm', '期中成绩'],
    '作业完成率': ['作业完成率', '作业完成比例', 'homework', '作业完成'],
    '期末考试分数': ['期末考试分数', '期末分数', 'final', '期末成绩']
}
...
def map_columns(df):
    ...
    for col in df.columns:
        ...
        for standard_col, variants in COLUMN_MAPPING.items():
            if col in variants or col.replace('（小时）', '') in variants:
                new_columns.append(standard_col)
                matched = True
                break
        if not matched:
            new_columns.append(col)  # 未匹配的列保留原名
...
def load_data_and_models():
    ...
    try:
        # 尝试多种编码加载
        encodings = ['gbk', 'utf-8-sig', 'utf-8', 'gb2312']
        df = None
        for encoding in encodings:
            try:
                df = pd.read_csv(file_name, encoding=encoding)
                st.success(f"成功使用 {encoding} 编码加载数据")
                break
            except:
                continue
        
        if df is None:
            st.error("所有编码尝试失败，无法加载数据")
        ...
        # 打印实际列名（方便排查）
        st.info(f"CSV文件实际列名（映射后）：{list(df.columns)}")
        ...
        if missing_cols:
            ...
            st.info(f"当前数据文件包含列：{list(df.columns)}")
            return None, None, None, None
...
def show_project_intro():
    ...
    with col_img:
        st.write("### 系统预览")
        # 替换为你的预览图路径，或注释掉这行避免报错
        # st.image("系统预览图.png", use_container_width=True)
        st.info("系统预览图占位（可替换为实际图片路径）")
    ...
    if submit_btn:
        try:
            ...
            if pred_score < 60:
                ...
                st.warning(f"成绩等级：{level}")
            elif 60 <= pred_score < 70:
                level = "及格"
                # st.image(emoji_paths[level], width=150)
                st.info(f"成绩等级：{level}")
            elif 70 <= pred_score < 90:
                level = "良好"
                # st.image(emoji_paths[level], width=150)
                st.success(f"成绩等级：{level}")
            else:
                level = "优秀"
                # st.image(emoji_paths[level], width=150)
                st.success(f"成绩等级：{level}")
